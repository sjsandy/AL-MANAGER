 ...
 * @author studio
 */
abstract class Basejump_Abstracts_Loader {
    ...
    function enqueue_scripts($resource = array(), $type = 'js') {
        if (!is_array($resource) OR empty($resource))
            return;

        if ($type == 'js'):
            foreach ($resource as $name => $uri) {
